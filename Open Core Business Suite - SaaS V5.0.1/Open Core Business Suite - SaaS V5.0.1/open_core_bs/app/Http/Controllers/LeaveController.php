...
class LeaveController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('leave.index', compact('leaveTypes'));
    }
    ...
    public function getLeaveBalanceForType($leaveTypeId)
    {
        ...
        $userLeaveBalance = UserAvailableLeave::where('user_id', $user->id)
    ...
    public function applyLeave()
    {
        ...
        $userLeaveBalances = UserAvailableLeave::where('user_id', $user->id)
            ->where('year', $currentYear)
            ->with('leaveType')
            ->get()
            ->keyBy('leave_type_id');
        ...
        foreach ($leaveTypes as $leaveType) {
            ...
            if ($balance) {
                ...
                // If no balance record exists, user has 0 leaves available
                // Do NOT show days_allowed as that gives false impression of available balance
                $leaveType->user_entitled = 0;
                $leaveType->user_carried = 0;
                $leaveType->user_additional = 0;
                $leaveType->user_used = 0;
                $leaveType->user_available = 0;
            }
        }

        return view('leave.apply', compact('leaveTypes', 'user'));
    }
    ...
    public function myBalance()
    {
        ...
        $balances = UserAvailableLeave::where('user_id', $user->id)
            ->where('year', $currentYear)
            ->with('leaveType')
            ->get()
            ->keyBy('leave_type_id');
        ...
        return view('leave.balance', compact('user', 'leaveTypes', 'balances'));
    }
    ...
    public function teamCalendar()
    {
        ...
        // Get team members based on reporting structure (manager's team)
        // If user is a manager, get their direct reports
        $teamMembers = User::where('reporting_to_id', $user->id)
            ->where('status', 'active')
            ->get();

        // If no direct reports, show user's own department colleagues
        ...
        // Always include the current user in the team
        if (! $teamMembers->contains('id', $user->id)) {
        ...
        $leaves = LeaveRequest::whereIn('user_id', $teamMembers->pluck('id'))
            ...
            ->with(['user.designation', 'leaveType'])
        ...
        return view('leave.team-calendar', compact('user', 'teamMembers', 'leaves', 'leaveTypes'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Set user_id to authenticated user only if not provided (allows admin to create leave for others)
            ...
            // Handle compensatory off usage
            $useCompOff = $request->boolean('use_comp_off');
            $compOffDaysUsed = 0;
            $compOffIds = [];

            if ($useCompOff && $request->filled('comp_off_ids')) {
                $requestedCompOffIds = json_decode($request->comp_off_ids, true);

                if (is_array($requestedCompOffIds) && ! empty($requestedCompOffIds)) {
                    // Verify comp offs are available for the TARGET user (not auth user)
                    $availableCompOffs = CompensatoryOff::where('user_id', $validated['user_id'])
                        ->whereIn('id', $requestedCompOffIds)
                        ->where('status', 'approved')
                        ->where('is_used', false)
                        ->where('expiry_date', '>=', now()->toDateString())
                        ->orderBy('worked_date', 'asc')
                        ->get();

                    if ($availableCompOffs->isEmpty()) {
                        \Illuminate\Support\Facades\DB::rollBack();

                        return Error::response(__('Selected compensatory offs are not available.'));
                    }

                    $compOffDaysUsed = min($totalDays, $availableCompOffs->sum('comp_off_days'));
                    $compOffIds = $availableCompOffs->pluck('id')->toArray();
                }
            }

            // Calculate days needed from leave balance (after comp-off usage)
            $daysNeededFromLeaveBalance = $totalDays - $compOffDaysUsed;

            // Create leave request
            $leaveRequest = new LeaveRequest($validated);
            $leaveRequest->use_comp_off = $useCompOff;
            $leaveRequest->comp_off_days_used = $compOffDaysUsed;
            $leaveRequest->comp_off_ids = ! empty($compOffIds) ? $compOffIds : null;
            ...
            if ($request->hasFile('document')) {
                try {
                    ...
                    $file->storeAs('public/uploads/leaverequestdocuments', $fileName);
                    $leaveRequest->document = 'uploads/leaverequestdocuments/'.$fileName;
                } catch (\Exception $e) {
            ...
            // Check leave balance only if days are needed from regular leave balance
            if ($daysNeededFromLeaveBalance > 0) {
                $currentYear = date('Y');
                $leaveBalance = UserAvailableLeave::where('user_id', $validated['user_id'])
                    ->where('leave_type_id', $validated['leave_type_id'])
                    ->where('year', $currentYear)
                    ->first();

                // If no leave balance record exists, user has 0 leaves
                if (! $leaveBalance) {
                    \Illuminate\Support\Facades\DB::rollBack();

                    return Error::response(__('No leave balance available for this leave type. Employee needs '.$daysNeededFromLeaveBalance.' days but has 0 days available.'));
                }

                // Check if available leaves are insufficient
                if ($leaveBalance->available_leaves < $daysNeededFromLeaveBalance) {
                    \Illuminate\Support\Facades\DB::rollBack();

                    return Error::response(__('Insufficient leave balance. Employee needs '.$daysNeededFromLeaveBalance.' days but has '.$leaveBalance->available_leaves.' days available.'));
                }
            }
            ...
            try {
                // Get the user's manager or HR department for notification
                // For now, we'll get users with leave approval permission
                $approvers = User::permission('hrcore.approve-leave')->get();
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $validated = $request->validate([
                'user_id' => [
                    // Rule::requiredIf(auth()->user()->can('hrcore.create-leave-for-others')), // PERMISSION CHECK TEMPORARILY DISABLED
                    'exists:users,id',
                ],
                'leave_type_id' => 'required|exists:leave_types,id',
    ...
    public function indexAjax(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('document', function ($leave) {
                if ($leave->document) {
                    $url = asset('storage/'.$leave->document);
    ...
    public function actionAjax(Request $request)
    {
        ...
        try {
            ...
            if ($validated['status'] == 'cancelled') {
                ...
                $leaveRequest->cancel($validated['adminNotes'] ?? null, true);
            } else {
                ...
                if ($validated['status'] == 'approved') {
                    $leaveRequest->updateUserLeaveBalance('approve');

                    // Mark comp-offs as used if this leave uses comp-off
                    $leaveRequest->markCompOffsAsUsed();
                }
            }

            // Send notification to employee about status change
            try {
                $leaveRequest->refresh(); // Reload to get updated relationships

                if ($validated['status'] === 'approved') {
                    $leaveRequest->user->notify(new LeaveApproved($leaveRequest));
                } elseif ($validated['status'] === 'rejected') {
                    $reason = $validated['adminNotes'] ?? null;
                    $leaveRequest->user->notify(new LeaveRejected($leaveRequest, $reason));
                }
            } catch (\Exception $e) {
    ...
    public function showPage($id)
    {
        ...
        if (! request()->ajax() && ! request()->wantsJson()) {
            // Get active leave types for the edit form
            $leaveTypes = LeaveType::where('status', 'active')->get();

            return view('leave.show', compact('id', 'leaveTypes'));
        }
    ...
    public function show($id)
    {
        ...
        $response = [
            ...
            // Compensatory Off Usage
            'use_comp_off' => $leaveRequest->use_comp_off,
            'comp_off_days_used' => $leaveRequest->comp_off_days_used,
            'comp_off_ids' => $leaveRequest->comp_off_ids,
            ...
            // PERMISSIONS TEMPORARILY DISABLED - ALL ACTIONS ENABLED
            // Permissions for current user
            'can_approve' => true, // auth()->user()->can('hrcore.approve-leave') || ($leaveRequest->user->reporting_to_id === auth()->id()),
            'can_reject' => true, // auth()->user()->can('hrcore.approve-leave') || ($leaveRequest->user->reporting_to_id === auth()->id()),
            'can_cancel' => in_array($statusValue, ['pending', 'approved']), // ($leaveRequest->user_id === auth()->id() || auth()->user()->can('hrcore.cancel-leave')) && in_array($statusValue, ['pending', 'approved']),
            'can_edit' => $statusValue === 'pending', // ($leaveRequest->user_id === auth()->id() || auth()->user()->can('hrcore.edit-leave')) && $statusValue === 'pending',
            'can_delete' => true, // auth()->user()->can('hrcore.delete-leave'),
        ];
    ...
    public function approve(Request $request, $id)
    {
        try {
            ...
            if ($leaveRequest->status->value !== 'pending') {
                return response()->json([
                    'status' => 'error',
                    'message' => __('Only pending leave requests can be approved'),
                ], 400);
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => __('Failed to approve leave request'),
            ], 500);
        }
    }
    ...
    public function reject(Request $request, $id)
    {
        ...
        try {
            ...
            if ($leaveRequest->status->value !== 'pending') {
                return response()->json([
                    'status' => 'error',
                    'message' => __('Only pending leave requests can be rejected'),
                ], 400);
            }
            ...
            // Send notification to employee after transaction commits
            try {
                $leaveRequest->user->notify(new LeaveRejected($leaveRequest, $request->input('reason')));
            } catch (\Exception $e) {
                Log::error('Failed to send leave rejection notification: '.$e->getMessage());
                // Don't fail the request if notification fails
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => __('Failed to reject leave request'),
            ], 500);
        }
    }
    ...
    public function cancel(Request $request, $id)
    {
        try {
            ...
            if (! in_array($leaveRequest->status->value, ['pending', 'approved'])) {
                return response()->json([
                    'status' => 'error',
                    'message' => __('Only pending or approved leave requests can be cancelled'),
                ], 400);
            }
            ...
            if ($leaveRequest->status->value === 'approved' && $leaveRequest->from_date->isPast()) {
                return response()->json([
                    'status' => 'error',
                    'message' => __('Cannot cancel approved leave that has already started or passed'),
                ], 400);
            }
            ...
            // PERMISSION CHECK TEMPORARILY DISABLED - ALWAYS ALLOW CANCELLATION
            $leaveRequest->cancel($request->input('reason', ''), true); // auth()->user()->can('hrcore.cancel-leave')

            DB::commit();

            // Send notification after transaction commits (if notification system exists)
            try {
                ...
                Log::error('Failed to send leave cancellation notification: '.$e->getMessage());
                // Don't fail the request if notification fails
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => __('Failed to cancel leave request'),
            ], 500);
        }
    }
    ...
    public function myLeaves()
    {
        ...
        return view('leave.my-leaves', compact('leaveTypes', 'statistics'));
    }
    ...
    public function createMyLeave()
    {
        ...
        return view('leave.my-create', compact('leaveTypes', 'leaveBalances'));
    }
    ...
    public function storeMyLeave(Request $request)
    {
        $request->validate([
            ...
            'comp_off_days_used' => 'nullable|numeric|min:0',
            'comp_off_ids' => 'nullable|json',
        ]);
        ...
        try {
            ...
            if ($useCompOff && $request->filled('comp_off_ids')) {
                ...
                if (is_array($requestedCompOffIds) && ! empty($requestedCompOffIds)) {
                    // Verify comp offs are available
                    $availableCompOffs = CompensatoryOff::where('user_id', auth()->id())
                        ->whereIn('id', $requestedCompOffIds)
                        ->where('status', 'approved')
                        ->where('is_used', false)
                        ->where('expiry_date', '>=', now()->toDateString())
                    ...
                    // Calculate actual comp off days available
                    $compOffDaysUsed = min($totalDays, $availableCompOffs->sum('comp_off_days'));
                    $compOffIds = $availableCompOffs->pluck('id')->toArray();
                }
            }

            // Calculate days needed from leave balance
            $daysNeededFromLeaveBalance = $totalDays - $compOffDaysUsed;

            // Check leave balance only if needed
            if ($daysNeededFromLeaveBalance > 0) {
                $currentYear = date('Y');
                ...
                // If no leave balance record exists, user has 0 leaves
                if (! $leaveBalance) {
                    return Error::response(__('No leave balance available for this leave type. You need '.$daysNeededFromLeaveBalance.' days but have 0 days available.'));
                }

                // Check if available leaves are insufficient
                if ($leaveBalance->available_leaves < $daysNeededFromLeaveBalance) {
                    return Error::response(__('Insufficient leave balance. You need '.$daysNeededFromLeaveBalance.' days but have '.$leaveBalance->available_leaves.' days available.'));
                }
            }
            ...
            if ($request->hasFile('document')) {
                try {
                    $file = $request->file('document');
                    $fileName = time().'_'.$file->getClientOriginalName();
                    $path = $file->storeAs('leaverequestdocuments', $fileName, 'public');
                    $documentPath = $path;
                } catch (\Exception $e) {
                    Log::error('Document upload failed: '.$e->getMessage());
                    // Continue without document - it's optional
            ...
            $leaveRequest = LeaveRequest::create([
                ...
                'comp_off_ids' => ! empty($compOffIds) ? $compOffIds : null,
                'status' => LeaveRequestStatus::PENDING,
            ]);

            DB::commit();

            // Send notification to approvers (HR, Admin, Manager, and reporting manager)
            try {
                \App\Helpers\NotificationHelper::notifyAdminHR(new NewLeaveRequest($leaveRequest));
            } catch (\Exception $e) {
                Log::error('Failed to send leave request notification: '.$e->getMessage());
                // Don't fail the request if notification fails
            }
    ...
    public function cancelMyLeave(Request $request, $id)
    {
        ...
        try {
            ...
            // Use the model's cancel method to properly release Comp Offs
            $leaveRequest->cancel($request->reason ?? 'Cancelled by employee', false);
    ...
    public function myLeaveBalance()
    {
        ...
        return view('leave.my-balance', compact('leaveBalances', 'leaveTypes'));
    }
}
