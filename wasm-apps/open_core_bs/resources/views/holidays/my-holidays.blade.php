...
@section('vendor-style')
  ...
  <style>
    ...
    /* Ensure event text is readable in both light and dark mode */
    .fc-event .fc-event-main {
      color: #fff !important;
    }

    /* List view text color */
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    @if($totalHolidays === 0)
      <div class="alert alert-warning d-flex align-items-center" role="alert">
        ...
        <div>
          <strong>{{ __('No holidays found for') }} {{ $currentYear }}</strong><br>
          <small>{{ __('Please contact your administrator to add holiday information.') }}</small>
        </div>
      </div>
    @endif
              ...
              @empty
                <tr>
                  <td colspan="5" class="text-center text-muted py-4">
                    ...
                    <p class="mb-0">{{ __('No holidays available for') }} {{ $currentYear }}</p>
                  </td>
                </tr>
              @endforelse
            </tbody>
  ...
  @php
    $holidayData = $holidays->map(function($holiday) {
      return [
        ...
        'allDay' => true,
        'backgroundColor' => $holiday->color ?? ($holiday->date->isPast() ? '#6c757d' : ($holiday->date->isToday() ? '#28a745' : '#007bff')),
        'borderColor' => $holiday->color ?? ($holiday->date->isPast() ? '#6c757d' : ($holiday->date->isToday() ? '#28a745' : '#007bff')),
        'textColor' => '#ffffff',
        'extendedProps' => [
          'notes' => $holiday->notes,
          'code' => $holiday->code,
