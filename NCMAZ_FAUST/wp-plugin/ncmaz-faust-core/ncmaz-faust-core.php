...
/*
* Load plugin textdomain and notification check
*/
add_action('init', function () {
    load_plugin_textdomain('ncmaz-faust-core', false, dirname(plugin_basename(__FILE__)) . '/languages');

    // Check for required plugins
    if (!defined('WPGRAPHQL_CONTENT_BLOCKS_VERSION')) {
        add_action('admin_notices', function () {
            $class    = 'notice notice-error';
            $message1 = 'The Ncmaz Faust Core plugin is not working correctly.';
            ...
            $message2 = sprintf('Please install and activate the %s plugin.', $link);
